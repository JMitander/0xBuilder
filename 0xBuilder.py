...
class Configuration:
    ...
    def _get_env_variable(self, variable_name: str, default: Optional[str] = None) -> str:
        try:
    ...
    def _load_coinmarketcap_api_key(self) -> str:
        return self._get_env_variable("COINMARKETCAP_API_KEY")

    def _load_cryptocompare_api_key(self) -> str:
        return self._get_env_variable("CRYPTOCOMPARE_API_KEY")
    ...
    async def _load_ml_models(self) -> None:
        self.ML_MODEL_PATH = await self._get_env_variable("ML_MODEL_PATH")
        self.ML_TRAINING_DATA_PATH = await self._get_env_variable("ML_TRAINING_DATA_PATH")
        if not os.path.exists(self.ML_MODEL_PATH): 
            os.makedirs(self.ML_MODEL_PATH)
        if not os.path.exists(self.ML_TRAINING_DATA_PATH):
            os.makedirs(self.ML_TRAINING_DATA_PATH)
