...
async def main():
    """Main entry point with proper setup, error handling, and UI synchronization."""
    logger = None
    try:
        # Setup logging first
        await setup_logging()
        ...
        # Initialize the main bot without starting it
        xplorer = Xplorer(config, logger)
        
        # Initialize UI
        ui = Xplorer_UI(xplorer, logger)
        
        # Start UI task
        ui_task = asyncio.create_task(ui.start())
        
        # Wait for user to start bot from UI
        await ui.start_event.wait()
        ...
        # Start bot operation
        bot_task = asyncio.create_task(xplorer.run())
        
        # Wait for both tasks
        try:
            await asyncio.gather(ui_task, bot_task)
        except asyncio.CancelledError:
            ...
            await asyncio.gather(ui_task, bot_task, return_exceptions=True)
